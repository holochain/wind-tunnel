    ...
    ($self:ident, $metric:expr, $($field:ident),+) => {
        {
            ...
            $(
                if let Some(value) = $self.$field {
                    m = m.add_field(stringify!($field), value);
                }
            )+
            m
        }
...
pub trait ReportFields {
    /// Returns the associated [`WriteQuery`] for this host metrics fields.
    fn report_fields(&self, metric: WriteQuery) -> WriteQuery;
}
...
impl ReportFields for CpuMetrics {
    fn report_fields(&self, metric: WriteQuery) -> WriteQuery {
        metric
            .add_field("usage_guest", self.usage_guest)
            .add_field("usage_guest_nice", self.usage_guest_nice)
            .add_field("usage_idle", self.usage_idle)
            .add_field("usage_iowait", self.usage_iowait)
            .add_field("usage_irq", self.usage_irq)
            .add_field("usage_nice", self.usage_nice)
            .add_field("usage_softirq", self.usage_softirq)
            .add_field("usage_steal", self.usage_steal)
            .add_field("usage_system", self.usage_system)
            .add_field("usage_user", self.usage_user)
    }
}
...
impl ReportFields for DiskMetrics {
    fn report_fields(&self, metric: WriteQuery) -> WriteQuery {
        metric
            .add_field("free", self.free)
            .add_field("inodes_free", self.inodes_free)
            .add_field("inodes_total", self.inodes_total)
            .add_field("inodes_used", self.inodes_used)
            .add_field("inodes_used_percent", self.inodes_used_percent)
            .add_field("total", self.total)
            .add_field("used", self.used)
            .add_field("used_percent", self.used_percent)
    }
}
...
impl ReportFields for DiskioMetrics {
    fn report_fields(&self, metric: WriteQuery) -> WriteQuery {
        metric
            .add_field("io_time", self.io_time)
            .add_field("iops_in_progress", self.iops_in_progress)
            .add_field("merged_reads", self.merged_reads)
            .add_field("merged_writes", self.merged_writes)
            .add_field("read_bytes", self.read_bytes)
            .add_field("read_time", self.read_time)
            .add_field("reads", self.reads)
            .add_field("weighted_io_time", self.weighted_io_time)
            .add_field("write_bytes", self.write_bytes)
            .add_field("write_time", self.write_time)
            .add_field("writes", self.writes)
    }
}
...
impl ReportFields for KernelMetrics {
    fn report_fields(&self, metric: WriteQuery) -> WriteQuery {
        metric
            .add_field("boot_time", self.boot_time)
            .add_field("context_switches", self.context_switches)
            .add_field("entropy_avail", self.entropy_avail)
            .add_field("interrupts", self.interrupts)
            .add_field("processes_forked", self.processes_forked)
    }
}
...
impl ReportFields for MemMetrics {
    fn report_fields(&self, metric: WriteQuery) -> WriteQuery {
        metric
            ...
            .add_field("available", self.available)
            .add_field("available_percent", self.available_percent)
            .add_field("buffered", self.buffered)
            .add_field("cached", self.cached)
            .add_field("commit_limit", self.commit_limit)
            .add_field("committed_as", self.committed_as)
            .add_field("dirty", self.dirty)
            .add_field("free", self.free)
            .add_field("high_free", self.high_free)
            .add_field("high_total", self.high_total)
            .add_field("huge_page_size", self.huge_page_size)
            .add_field("huge_pages_free", self.huge_pages_free)
            .add_field("huge_pages_total", self.huge_pages_total)
            .add_field("inactive", self.inactive)
            .add_field("low_free", self.low_free)
            .add_field("low_total", self.low_total)
            .add_field("mapped", self.mapped)
            .add_field("page_tables", self.page_tables)
            .add_field("shared", self.shared)
            .add_field("slab", self.slab)
            .add_field("sreclaimable", self.sreclaimable)
            .add_field("sunreclaim", self.sunreclaim)
            .add_field("swap_cached", self.swap_cached)
            .add_field("swap_free", self.swap_free)
            .add_field("swap_total", self.swap_total)
            .add_field("total", self.total)
            .add_field("used", self.used)
            .add_field("used_percent", self.used_percent)
            .add_field("vmalloc_chunk", self.vmalloc_chunk)
            .add_field("vmalloc_total", self.vmalloc_total)
            .add_field("vmalloc_used", self.vmalloc_used)
            .add_field("write_back", self.write_back)
            .add_field("write_back_tmp", self.write_back_tmp)
    }
}
...
impl ReportFields for NetstatMetrics {
    fn report_fields(&self, metric: WriteQuery) -> WriteQuery {
        metric
            .add_field("tcp_close", self.tcp_close)
            .add_field("tcp_close_wait", self.tcp_close_wait)
            .add_field("tcp_closing", self.tcp_closing)
            .add_field("tcp_established", self.tcp_established)
            .add_field("tcp_fin_wait1", self.tcp_fin_wait1)
            .add_field("tcp_fin_wait2", self.tcp_fin_wait2)
            .add_field("tcp_last_ack", self.tcp_last_ack)
            .add_field("tcp_listen", self.tcp_listen)
            .add_field("tcp_none", self.tcp_none)
            .add_field("tcp_syn_recv", self.tcp_syn_recv)
            .add_field("tcp_syn_sent", self.tcp_syn_sent)
            .add_field("tcp_time_wait", self.tcp_time_wait)
            .add_field("udp_socket", self.udp_socket)
    }
}
...
impl ReportFields for ProcessesMetrics {
    fn report_fields(&self, metric: WriteQuery) -> WriteQuery {
        metric
            .add_field("blocked", self.blocked)
            .add_field("dead", self.dead)
            .add_field("idle", self.idle)
            .add_field("paging", self.paging)
            .add_field("running", self.running)
            .add_field("sleeping", self.sleeping)
            .add_field("stopped", self.stopped)
            .add_field("total", self.total)
            .add_field("total_threads", self.total_threads)
            .add_field("unknown", self.unknown)
            .add_field("zombies", self.zombies)
    }
}
...
impl ReportFields for SystemMetrics {
    fn report_fields(&self, mut metric: WriteQuery) -> WriteQuery {
        ...
        if let Some(uptime_format) = &self.uptime_format {
            metric = metric.add_field("uptime_format", uptime_format.clone());
        }
        metric
    }
}
