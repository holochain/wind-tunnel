...
use holochain_types::prelude::{AgentPubKey, ActionHash};
...
fn agent_behaviour_write(
    ...
    let _: ActionHash = call_zome(ctx, "agent_activity", "create_sample_entry", "this is a test entry value")?;
    Ok(())
}
...
fn agent_behaviour_get_agent_activity(
    ...
    let saved_write_peer = ctx.get_mut()
        .scenario_values
        .write_peer
        .clone();
    ...
    match saved_write_peer {
        Some(write_peer) => {
            let now = Instant::now();
            let activity: AgentActivity = call_zome(ctx, "agent_activity", "get_agent_activity_full", write_peer.clone())?;
        ...
        _ => {
            ...
            let new_peers: Vec<AgentPubKey> = ctx
                .runner_context()
                .executor()
            ...
            ctx.get_mut()
                .scenario_values
                .write_peer = new_peers.first().map(|p| p.clone());
        }
    }
