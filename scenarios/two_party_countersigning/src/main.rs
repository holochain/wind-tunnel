...
use holochain_types::prelude::{AgentPubKey, CellId, PreflightResponse};
...
use holochain_types::dna::EntryHash;
        ...
        .execute_in_place(
            async move {
                match initiate_with_peers {
                    ...
                    Some(agent_pub_key) => {
                        // Prove that our chain is unlocked by doing a write operation
                        // client.call_zome(
                        //     app_port,
                        //     cell_id.clone(),
                        //     "countersigning",
                        ...
                        //     (),
                        //     None,
                        // ).await.context("Failed to write to source chain")?;
                        ...
                        log::info!("Initiated a countersigning session with agent [{:?}] with session times [{:?}]", agent_pub_key, session_times);
                        ...
                        loop {
                            ...
                            match signal {
                                Some(signal) => {
                                    ...
                                    let other_agent_chain_top = other_response.agent_state.chain_top();
                                    log::info!("The other party [{:?}] has chain top [{:?}]", agent_pub_key, other_agent_chain_top);

                                    log::debug!("The other party [{:?}] has accepted the countersigning session.", agent_pub_key);

                                    log::info!("Submitting my session {:?} at {:?}: [{:?}]", cell_id.agent_pubkey(), Local::now(), my_preflight_response);
...
async fn complete_session(
    ...
    for i in 0.. {
        let r = client
            .call_zome(
                ...
                "countersigning",
                "commit_two_party",
                vec![initiate_preflight_response.clone(), participate_preflight_response.clone()],
                None,
            )
            .await
    ...
    await_countersigning_success(client.clone(), initiate_preflight_response.request.app_entry_hash, session_timeout)
        .await
        .with_context(|| {
            format!("Session between [{:?}] did not complete within the session time", participate_preflight_response.request.signing_agents)
        })?;

    Ok(retry_count)
}

async fn await_countersigning_success(client: TryCPClient, session_entry_hash: EntryHash, session_timeout: Instant) -> HookResult {
    loop {
        ...
        match signal {
            Some(signal) => {
                match rmp_serde::decode::from_slice::<Signal>(&signal.data).map_err(|e| {
                    ...
                    Signal::System(SystemSignal::SuccessfulCountersigning(eh)) if eh == session_entry_hash => {
