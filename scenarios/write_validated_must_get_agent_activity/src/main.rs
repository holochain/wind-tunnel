...
const GET_RANDOM_PEER_CLIENT_NAME: &str = "get_random_peer_client";
...
fn agent_behaviour_must_get_agent_activity(
    ...
    let Some(write_peer) = ctx.get().scenario_values.write_peer.clone() else {
        let client = ctx
            .get()
            .app_websocket_client(GET_RANDOM_PEER_CLIENT_NAME)?;

        log::debug!("Fetching random write peer agent...");
        ctx.get_mut().scenario_values.write_peer = call_zome_with_client(
            ctx,
            &client,
            "validated_must_get_agent_activity",
...
fn main() -> WindTunnelResult<()> {
    ...
    .with_app_websocket_client(GET_RANDOM_PEER_CLIENT_NAME, Arc::new(websocket_config))
