...
pub struct ScenarioValues {
    ...
    last_observed_chain_head: (u32, i64),
...
fn agent_setup(
    ...
    ctx.get_mut().scenario_values.last_observed_chain_head = (0, Timestamp::now().0);
        ...
        Some(write_peer) => {
            ...
            if let Ok(activity) = activity_result {
                ...
                let highest_observed = activity
                    .highest_observed
                    .map_or(previous_observed_chain_head.0, |v| v.action_seq);

                // If it's a new chain head, record the time difference between the new one and the previous one
                // (or divide by n if there are more than one chain head since).
                // Note that the chain head may decrease again in case the agent activity gets fetched
                // from a different peer than in the previous invocation of the behaviour, therefore
                // a sturating subtraction is needed here.
                let n = highest_observed.saturating_sub(previous_observed_chain_head.0);
                let now = Timestamp::now().0;
                if n > 0 {
                    let time_delta_s = (now as f64 - previous_observed_chain_head.1 as f64) / 1e6;
                    reporter.add_custom(
                        ReportMetric::new("mixed_arc_get_agent_activity_new_chain_head_delay")
                            .with_field("value", time_delta_s),
                    );
                }

                reporter.add_custom(
                    ReportMetric::new("mixed_arc_get_agent_activity_highest_observed_action_seq")
                ...
                ctx.get_mut().scenario_values.last_observed_chain_head = (highest_observed, now);
            } else {
                let metric = ReportMetric::new("mixed_arc_get_agent_activity_retrieval_error")
