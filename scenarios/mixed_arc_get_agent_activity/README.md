...
- _zero_read_: Selects a single 'zero-write' agent for the duration of the scenario run and queries the agent's activity.
  ...
  - `wt.custom.mixed_arc_get_agent_activity_new_chain_head_delay`: If a new chain head is observed, the time difference between now and when the previous chain head was observed (in case the chain head jumped by more than one the average time difference per chain item).
