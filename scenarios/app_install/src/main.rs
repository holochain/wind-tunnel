...
fn agent_setup(ctx: &mut AgentContext<HolochainRunnerContext, HolochainAgentContext<ScenarioValues>>,) -> HookResult {
    let admin_url = ctx.runner_context().get_connection_string();
    let reporter = ctx.runner_context().reporter();
    let admin_client = ctx.runner_context().executor().execute_in_place(async move {
        let admin_client = AdminWebsocket::connect(admin_url, reporter).await?;
        Ok(admin_client)
    })?;

    println!("Setting up an agent");
...
fn install_app_behaviour(ctx: &mut AgentContext<HolochainRunnerContext, HolochainAgentContext<ScenarioValues>>, happ_path: PathBuf, happ_name: &str) -> HookResult {
    ...
    ctx.runner_context().executor().execute_in_place(async move {
        admin_client.uninstall_app(installed_app_id).await.map_err(|e| {
            anyhow::anyhow!("Failed to uninstall app: {:?}", e)
        })?;
