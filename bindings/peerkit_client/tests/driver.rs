...
async fn waits_for_send_completion_events() {
    ...
    // A reported send failure is answered by the CLI, so it is returned to the
    // caller and only the second, unclaimed `Send failed:` line is counted as
    ...
    // That failure is recoverable: the node keeps sending afterwards.
