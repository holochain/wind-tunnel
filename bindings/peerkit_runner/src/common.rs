...
fn write_identity_file(agent_id: &str, seed: &[u8; 32]) -> anyhow::Result<PathBuf> {
    ...
    if let Err(error) = file.write_all(seed).context("failed to write identity key") {
        drop(file);
        if let Err(cleanup_error) = std::fs::remove_file(&path) {
            log::warn!(
                "failed to remove incomplete identity key {path}: {cleanup_error}",
                path = path.display()
            );
        }
        return Err(error);
...
pub fn take_received_messages(
    ...
    let Some(node) = ctx.get().node.clone() else {
        return Ok(Vec::new());
    };
    ctx.runner_context()
        .executor()
