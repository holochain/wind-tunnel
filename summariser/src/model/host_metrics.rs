...
pub struct HostMetrics {
    ...
    pub holochain_process: Option<ProcessMetrics>,
...
pub struct CpuMetrics {
    ...
    pub p99: f64,
    /// Number of hosts that spent any time above 80% total CPU usage during the run
    pub hosts_above_80_percent: usize,
    /// Mean time (seconds) spent above 80% total CPU usage, averaged over only those hosts
    /// that exceeded the threshold. Zero if no host exceeded it.
    pub mean_time_above_80_percent_s: f64,
}
...
pub struct MemMetrics {
    ...
    /// Maximum per-host mean memory used percent (0–100).
    ///
    /// Computed as the max of each host's mean `used_percent` over the run.
    /// More diagnostic than the fleet-wide mean for heterogeneous hardware:
    /// catches a single host under memory pressure even when others are fine.
    pub max_host_used_percent: f64,
    /// Maximum swap used percent observed on any single host (0–100).
    ///
    /// Computed per-host as `(swap_total - swap_free) / swap_total * 100`,
...
pub struct NetMetrics {
    ...
    ///
    /// For each host the interface carrying the most bytes (recv+sent) is selected as "primary".
    /// Counter derivatives and byte rates are computed per-host before aggregation,
    /// so multi-host values are correct.
...
pub struct DiskSpace {
    /// Number of hosts where mean used percent for this mount point exceeded 90%
    pub hosts_at_risk: usize,
    /// Total number of hosts that reported data for this mount point
...
pub struct SystemLoadMetrics {
    /// Mean 1-minute load average across all hosts.
    ///
    /// **Raw, unnormalised value** — not divided by `n_cpus`. Meaningful primarily for
    /// homogeneous clusters where all machines have the same core count. Use
    /// [`overloaded_percent`][SystemLoadMetrics::overloaded_percent] for a hardware-normalised
    /// signal that accounts for varying CPU counts per host.
    pub load_1min: f64,
    /// Mean 5-minute load average across all hosts.
    ///
    /// **Raw, unnormalised value** — not divided by `n_cpus`. See [`load_1min`][SystemLoadMetrics::load_1min]
    /// for caveats. Use [`overloaded_percent`][SystemLoadMetrics::overloaded_percent] for a
    /// hardware-normalised signal.
    pub load_5min: f64,
    /// Mean 15-minute load average across all hosts.
    ///
    /// **Raw, unnormalised value** — not divided by `n_cpus`. See [`load_1min`][SystemLoadMetrics::load_1min]
    /// for caveats. Use [`overloaded_percent`][SystemLoadMetrics::overloaded_percent] for a
    /// hardware-normalised signal.
...
/// Process metrics aggregated across all matching processes and hosts.
///
/// Sourced from telegraf's `inputs.procstat` plugin.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct ProcessMetrics {
    /// CPU usage percentage across all matched processes
