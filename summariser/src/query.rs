...
/// Query pre-aggregated (interval-based) instrument data for a specific operation.
///
/// When the reporter uses OTel with a PeriodicReader, each row in InfluxDB contains
/// aggregated histogram fields (`count`, `sum`, `min`, `max`, `mean`) for one export
/// interval instead of individual event values. Use [`crate::analyze::aggregated_timing_stats`]
/// to compute summary statistics from the returned DataFrame.
#[allow(dead_code)]
pub async fn query_instrument_data_aggregated(
    client: influxdb::Client,
    summary: &RunSummary,
    operation_id: &str,
) -> anyhow::Result<DataFrame> {
    const TABLE: &str = "wt.instruments.operation_duration";
    let q = ReadQuery::new(format!(
        r#"SELECT count, sum, min, max, mean FROM "windtunnel"."autogen"."{TABLE}" WHERE run_id = '{}' AND operation_id = '{}' AND is_error = 'false'"#,
        summary.run_id, operation_id
    ));
    log::debug!("Querying (aggregated): {q:?}");
    ...
    log::trace!("Loaded aggregated frame: {frame}");

    Ok(frame)
}

/// Query pre-aggregated zome call instrument data (both app_call_zome and trycp_app_call_zome).
///
/// Returns a DataFrame with `count`, `sum`, `min`, `max`, `mean` plus tag columns
/// `zome_name`, `fn_name`, `agent`.
#[allow(dead_code)]
pub async fn query_zome_call_instrument_data_aggregated(
    client: influxdb::Client,
    summary: &RunSummary,
) -> anyhow::Result<DataFrame> {
    const TABLE: &str = "wt.instruments.operation_duration";
    let q = ReadQuery::new(format!(
        r#"SELECT count, sum, min, max, mean, zome_name, fn_name, agent FROM "windtunnel"."autogen"."{TABLE}" WHERE run_id = '{}' AND (operation_id = 'app_call_zome' OR operation_id = 'trycp_app_call_zome') AND is_error = 'false'"#,
        summary.run_id
    ));
    log::debug!("Querying (aggregated zome calls): {q:?}");
    ...
    log::trace!("Loaded aggregated zome call frame: {frame}");
